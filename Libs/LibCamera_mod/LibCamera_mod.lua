...
-- LIBCAMERA --
---------------
-- cosFix: This is https://github.com/Mpstark/LibCamera with some modifications.
-- I added a "not_really" argument to the StopZooming() function
-- to skip the final call of reallyStopZooming(), such that I can still
-- manually zoom in the same frame.
-- I also added the ZoomInProgress() function to check from outside if
-- a zoom easing is currently in progress. Other than that I just added
-- some comments to help me understand the code better.

local MAJOR, MINOR = "LibCamera_mod-1.0", 2;
...
function LibCamera:SetZoom(endValue, duration, easingFunc, callback)
    ...
    local func = function()
        ...
        if ((beginTime + duration > currentTime) and not beyondPosition) then
            ...
            if (callback) then
                -- cosFix: Not necessary to call callback() here, because it was already called as easingZoom.callback
                -- by StopZooming() above.
                -- callback();
...
-- cosFix: I added this "not_really" argument to skip the final call of reallyStopZooming().
...
-- cosFix: Need this function to see if zooming is in progress.
